...
import random
from .mu_coma_lambda_roulette_ga import MuComaLambdaRouletteGA
from search import PopulationSearchAlgorithm, BinarySolution, sort_pop, eval_solutions

class MuComaLambdaTournoi(MuComaLambdaRouletteGA):
    ...
    def __init__ (self, prob, options) :
        ...
        * Un dictionnaire des paramètres des algorithmes 
        ...
        """
        MuComaLambdaRouletteGA.__init__(self, prob, options)
        
    
    @property
    def name(self):
        return "µ,λ(tournoi) µ{}_λ{}_pc{}_pm{}_type{}".format(self._mu, self._lambda, self._pc, self._pm, self._type_cdf).replace('.',',')
